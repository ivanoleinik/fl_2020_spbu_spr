...
\begin{enumerate}
  ...
  \begin{solution}
  	Пусть $L$ -- язык из условия. Докажем с помощью леммы, что $L$ неавтоматный. По данному $n$ возьмем слово $\alpha = b^naab^na^n \in L$. $(\alpha =uvw) \land (|v|\geqslant 1) \land (|uv|\leqslant n) \Rightarrow v = b^k$, где $k > 0$. При $i = 0$: $uv^iw = uw = b^{n - k}aab^na^n \not \in L$, так как $n - k < n$.
  \end{solution}
\end{enumerate}
