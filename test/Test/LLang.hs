...
shrek :: (Eq a, Show a) => Parser String String a -> String -> a -> Assertion
shrek p input a =
...
unit_parseVar = do
  runParser parseVar "_data" @?= Success (toStream "" (Position 0 5)) "_data"
  runParser parseVar "foo\'\'" @?= Success (toStream "" (Position 0 5)) "foo\'\'"
  runParser parseVar "Zipp077" @?= Success (toStream "" (Position 0 7)) "Zipp077"
  runParser parseVar "a_1_b_2 c_3" @?= Success (toStream " c_3" (Position 0 7)) "a_1_b_2"
...
unit_parseIf = do
  ...
  shrek parseIf "if (x/=y) ./ read a; write(a); \\. ./ \\."
                  (If
...
unit_parseWrite = do
  ...
  shrek parseWrite "write (x^y)" (Write (BinOp Pow (Ident "x") (Ident "y")))
...
unit_parseReturn = do
  shrek parseReturn "up (1+2)" (BinOp Plus (Num 1) (Num 2))
  shrek parseReturn "up (a)" (Ident "a")
  shrek parseReturn "up (foo(a, 5))" (FunctionCall "foo" [Ident "a", Num 5])
...
unit_parseDef = do
  shrek parseDef "fun id: n ./\\. up (id(4))"
    (Function "id" ["n"] (Seq []) (FunctionCall "id" [Num 4]))
  shrek parseDef "fun foo: a b ./\\. up (foo(a) + foo(b))"
    (Function "foo" ["a", "b"]
      (Seq [])
      (BinOp Plus
        (FunctionCall "foo" [Ident "a"])
        (FunctionCall "foo" [Ident "b"])
      )
    )
  assertBool "" $ isFailure (runParser parseDef "fun id: ./\\.")
...
unit_parseProg = do
  shrek parseProg "fun r: x ./ read x; \\. up (0) fun w: x ./ write(x); \\. up (0) ./ read x; write(r(x) || w(x)); \\."
    (Program [ Function "r" ["x"] (Seq [ Read "x" ]) (Num 0)
             , Function "w" ["x"] (Seq [ Write (Ident "x") ]) (Num 0)
             ]
             (Seq [ Read "x"
  ...
  shrek parseProg "fun add: x y ./\\. up (x + y)  ./ write (add(2, 3)); \\."
    (Program [ Function "add" ["x", "y"] (Seq []) (BinOp Plus (Ident "x") (Ident "y"))]
             (Seq [ Write (FunctionCall "add" [Num 2, Num 3])])
    )
  shrek parseProg "./ write(11); \\."
    (Program [] (Seq [ Write (Num 11)]))
  shrek parseProg "fun bar: a b c ./\\. up (0) ./\\."
    (Program [Function "bar" ["a", "b", "c"] (Seq []) (Num 0)] (Seq []))
  assertBool "" $ isFailure (runParser parseProg "fun kek: ./ write(x); \\. up(0) ./\\.")
  assertBool "" $ isFailure (runParser parseProg "fun if: x ./\\. up(0) ./\\.")
  assertBool "" $ isFailure (runParser parseProg "fun foo: a b ./ read a; write(a); \\. up(0) ./ write(b); \\.")
  assertBool "" $ isFailure (runParser parseProg "fun : x ./\\. up (x) ./\\.")
  assertBool "" $ isFailure (runParser parseProg "fun foo: x ./ bind y (x); \\. up (z) ./\\.")
...
unit_Position = do
  runParser parseDef "fun foo: a b ./\\. up (foo(a) + foo(b))" @?=
    Success (toStream "" (Position 0 38))
    (Function "foo" ["a", "b"]
      (Seq [])
      (BinOp Plus
        (FunctionCall "foo" [Ident "a"])
  ...
  runParser parseL "./\twrite\n(1^2); \\." @?=
    ...
    (Seq [Write (BinOp Pow (Num 1) (Num 2))])